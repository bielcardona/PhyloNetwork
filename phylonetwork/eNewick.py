...
def makeeNewickParser():
    ...
    lsqparen  = Literal("[").suppress()
    rsqparen  = Literal("]").suppress()
    ...
    name = Word(alphanums + alphas8bit +
                "_" + "-" + "." + "+" + "&" + "/" + "~" + "{" + "}" + "*" + "'" + '"' + '\\' + '?')
    string = Word(alphas)
    metadata_string = Word(alphanums + alphas8bit +
                           "_" + "-" + "." + "+" + "&" + "/" + "~" + "{" + "}" + "*" + "'" + '"' + '\\' + '?' +
                           ',' + '=')
    fnumber = Combine(
        Word("+-"+nums, nums) + 
    ...
    label = (
        ...
        Optional(lsqparen + metadata_string + rsqparen).setResultsName("node_metadata") +
        Optional(
            colon +
            Optional(lsqparen + metadata_string + rsqparen).setResultsName("arc_metadata") +
            Optional(fnumber).setResultsName("length")
        )
    )
...
if __name__ == '__main__':
    text='((1,,(2)h#LTG1:1.5)x,(h1,3:5.5)y[nmd]:5.76)r;'
#    text='xx#3'
